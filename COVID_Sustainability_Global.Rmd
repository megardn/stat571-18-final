...
pacman::p_load(tidyverse, dplyr, ggplot2, ggthemes, data.table, lubridate,
               ...
               plotly, ggpubr, vistime, skimr, glmnet, dgCMatrix, leaps, car, keras, neuralnet, tensorflow, missForest, fastDummies)
```
...
```{r}
# latest data for each country, restricted to 2016+
...
wsd.df_list2 <- list(wsd.latest, wsd.avg) #dfs to merge
wsd.sum.wide2 <- wsd.df_list2 %>% reduce(full_join, by='country')
#correlations
wsd.corr2 <- wsd.sum.wide2 %>% 
  select_if(., is.numeric) %>%
  cor(., use="pairwise.complete.obs")
#removing upper triangle
wsd.corr2[upper.tri(wsd.corr2)] <- 0
diag(wsd.corr2) <- 0
wsd.corr2.list <-  apply(wsd.corr2, 2, function(x) any(abs(x) > 0.97))
data.wsd2.nocorr <- wsd.sum.wide2[ , !wsd.corr2.list]


wsd.avgcorr2 <- wsd.avg %>% 
  select_if(., is.numeric) %>%
  cor(., use="pairwise.complete.obs")
#removing corr from wsd avg
wsd.avgcorr2[upper.tri(wsd.avgcorr2)] <- 0
diag(wsd.avgcorr2) <- 0
wsd.avgcorr2.list <-  apply(wsd.avgcorr2, 2, function(x) any(abs(x) > 0.97))
data.wsd.avg.nocorr <- wsd.avgcorr2[ , !wsd.avgcorr2.list]
