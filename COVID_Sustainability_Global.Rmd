...
wsd.change <- wsd %>% 
  ...
  #filter(row_number() == 1 | row_number() == n()) %>% #select first and last timepoint for each country - leads to inconsistent times
  mutate_each(funs(c(NA,diff(.))))%>% #calculate change
  select(-Year) %>%
  drop_na() %>%
  ungroup()
colnames(wsd.change)[2:25] <- paste(colnames(wsd.change)[2:25], "diff", sep = "_") #add suffix
#hist(wsd.change$Year)
```
