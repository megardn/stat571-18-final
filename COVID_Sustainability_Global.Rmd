...
Vaccine outcomes:
NOTE - trying to extract the date that vaccinations started in each country doesn't yield accurate results since some countries were pretty far into vaccinating people before reporting. See comments below and the `vaccination_events  ` chunk in covid_EDA.Rmd.

```{r}
# creating dfs of vaccination events - much of this is from covid_EDA.Rmd and can be removed/cleaned up once Rmds are merged!
## date of vac_start doesn't actually mean the 1st day vaccines were given, just the first day a country reported and were in the process of giving vaccines!
## many countries dont have data until a while after starting, ie 70% pop is already vaccinated at day 1

vac_start_df <- covid %>%
  select(country, date, vaccination_events, binned_people_vaccinated_per_hundred, people_vaccinated) %>%
...
vac_rate <- covid %>%
  ...
  filter(daily_vaccinations_per_million > 0,
         ...
         date <= "2021-06-30") %>% #daily vaccinations in 1st 6 mo. of 2021 - best i could think to do w/o accurate vaccine start dates
...
``` {r}
# cases & deaths (couldn't subset to prevaccines in each country, see above)
covid_prevac <- covid %>%
  group_by(country) %>%
  subset(date < mean(vac_start_df$date.first.vac)) %>% #setting mean of all reported vac_start days ~ don't love it but maybe in aggregate they're representative
  arrange(country,date) %>%
  slice_tail(n=1) %>%
  ungroup() %>%
...
covid_maxrate <- covid %>%
  ...
  rename(daily.new.cases.per.100_max = daily_new_cases_per_hundred,
         ...
         weekly.new.deaths.per.100_max = weekly_new_deaths_per_hundred, # cap at pre-vaccine availability
...
# c1 <-unique(covid$country)
# c2 <- unique(covid.all$country)
#setdiff(c1, c2)
#Palau dropped from summary data - ok since it's not in WSD anyway
```
