...
pacman::p_load(tidyverse, dplyr, ggplot2, ggthemes, data.table, lubridate,
               ...
               plotly, ggpubr, vistime, skimr, glmnet, dgCMatrix, leaps, car, keras, neuralnet, tensorflow)
```
...
Subdividing our dataset into `data.train`(60%), `data.test` (20%), and `data.val` (20%) - QUESTION - ok to also drop Country here?
...
data.train <- as.data.frame(df[idx_train, -1])
data.test <- as.data.frame(df[idx_test, -1])
data.val <- as.data.frame(df[idx_val, -1 ])
data.train.test <- as.data.frame(df[-idx_val, -1]) #for methods that don't require separate test set
``` 
...
Setting x and y data as matrixes
```{r}
#may need to set dummy vars - just grabing numeric for now to test!
data.train.test <- data.train.test %>% select_if(is.numeric)

## training input/y: need to be matrix/vector
data_xtrain <- data.train.test[, -192]   #dim(data3_xtrain), remove y
data_ytrain <- data.train.test[, 192]   # find y
...
model <- keras_model_sequential() %>%
  layer_batch_normalization(input_shape = c(p)) %>% #recommended
...
fit.nn <- model %>% fit(
  ...
  epochs = 20,
  batch_size = 50,
  validation_split = .15 # set 15% of the data_xtain, data_ytrain as the validation data
)
