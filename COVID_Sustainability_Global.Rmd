...
pacman::p_load(tidyverse, dplyr, ggplot2, ggthemes, data.table, lubridate,
               ...
               rworldmap, RColorBrewer, classInt, matrixStats)
```
...
Linear Regression models based on economic factors, disease burden factors, and political factors, as well as a data-driven LASSO model, have converged on the following variables as significantly predictive of vaccination success:
* World Region
...
** Policy Recommendation: This may indicate a nation's gender-based social liberality. Aid should thus be considered for nations that are **needs end of sentence - ex: nations that are socially more conservative?**
* Regime Type
** Policy Recommendation: Mutual aid relationships should be facilitated between nations of varying regime types. 
* Number of Internet Users
** Internet access should be made widely available as part of aid packages. This may help to provide adequate information regarding vaccinations, and help coordinate a nation's vaccination logistics (i.e. online signups and messaging). 

Furthermore, we have developed more robust models that predict VSI with varying degrees of accuracy (Linear Regression & LASSO, Trees & Random Forest, and Artificial Neural Networks). While predictive accuracy was moderate for each model individually, by creating an ensemble model that averages the predictions of all models, we have achieved a robust predictive ability, that could help forecast vaccination success in subsequent COVID-19 vaccination campaigns and future pandemics. 

## Conclusion

We have developed a robust vaccination index capturing a country's past vaccination success and our models highlighted several political, economic, social and health-related factors that have a crucial influence on a country's ability to vaccinate its populace. However, this analysis comes with certain limitations. First, the dataset is constrained to countries for which a sufficient amount of data was present to compute both VSI and summary political, economic, social and health-related factors. While this was possible for a substantial number of countries (n = 99), some countries had to be dropped (n = 126 from the vaccination data, n = 42 from the world sustainability data) including countries such as China, which did not sufficiently report on vaccination data but would be a very important country to include. By obtaining more complete information, future studies may further improve the completeness of this dataset and thereby increase the accuracy of VSI predictions. Moreover, in addition to the factors found to influence VSI in this study, it is likely that other factors influence a country's vaccination success, including more psychosocial and individual characteristics related to beliefs around vaccinations. 

In conclusion, **out of inspiration rn, would be nice to have a little conclusion sentence here**



# Data Preprocessing

Before starting to do more involved EDA, we will preprocess our data to ensure we have wokable datasets to begin our exploration with. 

## WSD preprocessing

First, we will begin to process the World Sustainability Dataset. 
```{r, eval = FALSE}
data.raw <- read.csv('data/WorldSustainabilityDataset.csv', header = TRUE)
```

There are some missing elements labeled as empty instead of NA. Let's fix this before proceeding.
```{r, eval = FALSE}
...
```
After cleaning, we are left with `n.countries` countries, which is a workable amount. 
...
Next, we will read in COVID cases/deaths and vaccination data sets. We remove countries with too many NAs and generate new variables indicating cases and deaths per hundred, to account for the total population in each country.  
...
Load outputs from data cleaning:
...
wsd %>% mutate(country = as.character(country)) %>% 
  ...
  mutate(n_years = n()) %>%
  arrange(n_years) %>%
  ggplot() + geom_bar(aes(y = forcats::fct_reorder(country, -n_years), fill = country)) +
  theme(legend.position = "none") +
  ggtitle("Years of data reported for each country") +
  xlab("Number of years reported")
```

This is a limitation that should be kept in mind when interpreting the results of our model and the predictive power of averaged variables in particular, since an averaged GDP for one country might reflect 17 years of data while the averaged GDP of another may only reflect 3 years. 
...
First, we create a dataframe with the latest COVID data from each country. Every country has some data from March 17, 2022, though some countries' last reported vaccination data was from as early as `r unname(summary(last.timepoint$date.vac_last)[1])`.
...
Based on the distribution plot, we can see that Somalia appears to be the country with the lowest VSI, and Cuba appears to have the highest VSI. The world map furthermore indicates that generally, North and South America, Europe, Australia and Japan are some regions with higher VSI scores, whereas regions including Africa, Russia and some Asian countries seem to have lower VSI scores. Note that countries without any color are not included in this dataset as they did not report vaccination data required for the computation of VSI. 
...
In this section, we are using socioeconomic factors and pre-vaccination COVID cases and deaths from `case.wsd.wide` to predict Vaccination Success Index (VSI). Note that VSI corresponds the inverse signed PC1 scores and (based on the relative loadings of each raw vaccination value) indicates how well a country did at getting people vaccinated, i.e. a positive score indicates a good vaccination performance, while a negative score indicates a poor vaccination performance. 

## Data splitting

We'll start by subsetting our dataset of 99 countries into `data.train`(85%) and `data.val` (15%). These splits were chosen because our dataset has a relatively small number of predictors and our most robust modeling methods have means of estimating testing error (Random Forest's OOB error, Regsubsets Cp, Neural Networks' internal data.val split).
...
LASSO results using the `coef.1se` suggest again that life expectancy is an important predictor of `VSI`. In addition, the LASSO approach selected the 17 year average number of individuals using internet. When feeding these into a linear model, both have a significant effect on `VSI`.  

## Regsubsets

Running regsubsets to optimize a model by minimizing Cp. 
We do get a warning that regsubsets still finds our data to be colinear, so we re-run correlation reduction with 95% cutoff on `data.train` to whether we can build a more optimal model. While this alternate model does better in modeling VSI (see *Comparing Regsubsets Models* in the Appendix), we felt this improvement in performance was not worth the lack of comparability with LASSO, RF and other models built on the full `data.train` set of predictors.  
...
Now let's try some backward selection on this model:
...
Reducing to only significantly predictive variables, we find they are fairly similar to those identified by our full linear model. *make sure this is still true once Joelle refines*. This is reassuring to have our data-driven model identify similar predictors to our hypothesis-driven models. 
...
While an advantage of using a single tree is that we can see which variables are diving our prediction, namely, the leaves on our branching tree seen above, Random Forest may provide better results at the cost of interpretability. Accordingly, now we will tune and train a Random Forest model. 
...
The lowest MSE calculated on the internal validation sample (15% of `data.train` that's been cleaned, imputed and normalized for NN input) is `r fit.nn$metrics$val_loss[min.epoch]` at the `r min.epoch`. Now that we've defined our model, we can refit it without the internal validation split and using `r min.epoch` epochs.
...
Now we'll attempt to create the best model by bagging those created above.
...
As we can see, the RandomForest MSE is lowest while LASSO's MSE is highest. By weighting the contributions of each model to our final prediciton accordingly, we obtain a training MSE of `r weighted.train.mse`.
...
validaiton.df.yhat <- data.val %>% dplyr::select(VSI) #initialize df to store outputs
validaiton.df.yhat$full.lm <- predict(lm.full, data.val) 
validaiton.df.yhat$lasso <- predict(fit.1se.lm, data.val) 
validaiton.df.yhat$rf <- predict(fit.final.rf, data.val) 
validaiton.df.yhat$regsub <- predict(fit1.lm, data.val) 
nn.list.val <- model2 %>% predict(data_xval) %>% unname()
validaiton.df.yhat <- cbind(validaiton.df.yhat, as.data.frame(nn.list.val))
validaiton.df.yhat <- validaiton.df.yhat %>%
  mutate(weighted.pred = rowWeightedMeans(as.matrix(validaiton.df.yhat[ , c(2:6)]), w=weight.list, na.rm=TRUE))
weighted.val.mse <- mean((validaiton.df.yhat$VSI - validaiton.df.yhat$weighted.pred)^2)
weighted.val.mse
```
...
```{r}
#find correlation
val.cor_value <- cor(validaiton.df.yhat$VSI, validaiton.df.yhat$weighted.pred, use = "complete.obs")

plot_validation <- validaiton.df.yhat %>% 
  ...
  annotate("text", x = -3, y = max(validaiton.df.yhat["weighted.pred"]) - 0.5, # position
           size = 4, # font size
