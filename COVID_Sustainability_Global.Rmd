...
# PCA: Vaccination Status Index

In this section, we use selected vaccination variables from `vaccination_summary.csv` to see if we can come up with a good PC1 component that can capture moste of the variance in vaccination data. 
...
PC1 explains 73% of the variance and is therefore an excellent index indicating how well a country did at vaccinating their population. We are calling it the "Vaccination Success Index", i.e. `VSI`. 
...
Let's now merge the VSI score with our cases and world sustainability dataset. 

```{r}

#adding in PC1
yval <- new.data.with.VSI %>% dplyr::select("country", "VSI")
df <- inner_join(case.wsd.wide, yval,  by = ("country")) # ending up with 99 countries
#names(df)
n_unique(df$country)
...
Subdividing our dataset of 99 countries into `data.train`(60%), `data.test` (20%), and `data.val` (20%) 
```{r}
# Split the data 
N <- length(df$country)
n1 <- floor(.6*N)
...
A number of variables in our dataset appear highly correlated. This is due to several reasons:  

* some variables already initially present in the world sustainability dataset (`WSD`) were highly correlated (for example, the proportion of Urban and Rural population variables) 
* the creation of different summary measures over distinct time spans - e.g., last 2018 data, 2015-2018 average, 2001-2018 average, 2001-2018 change) also led to a number of variables that may contain very similar information. 

To address this, we propose two different approaches: 
...
* In the second approach, we are running more inclusive models on distinct `WSD` summary measures (2018 data, 3 year average data, etc.) and `COVID` summary measures to determine which summary measures may be most informative in predicting `VSI`. 
...
```{r}
# economic factors lm
lm.econ.1 <- lm(data = df, formula = VSI ~ ExportGoodsServices.GDP_avg + 
...
lm.econ.1.refined <- lm(data = df, formula = VSI ~ GDP.PerCapita.Current_avg)
...
* `cumulative_total_cases_per_hundred_last` - last reported total number of cases prior to vaccination start
* `cumulative_total_deaths_per_hundred_last` - last reported total number of deaths prior to vaccination start
...
* `monthly_new_cases_per_hundred_last` - last reported monthly case rate reached prior to vaccination start
* `monthly_new_deaths_per_hundred_last` - last reported monthly death rate reached prior to vaccination start
...
lm.covid.1 <- lm(VSI ~ cumulative_total_cases_per_hundred_prevac +
                   ...
                   cumulative_total_cases_per_hundred_last +
                   cumulative_total_deaths_per_hundred_last +
                   monthly.new.cases.per.100_max +
                   monthly.new.deaths.per.100_max +
                   monthly_new_cases_per_hundred_last +
                   monthly_new_deaths_per_hundred_last, data = data.train)
summary(lm.covid.1)

# remove non-significant variables
lm.covid.1.refined <- lm(VSI ~ cumulative_total_deaths_per_hundred_prevac +
                           cumulative_total_cases_per_hundred_last +
                           monthly_new_deaths_per_hundred_last, data = data.train)
summary(lm.covid.1.refined)

lm.covid.1.refined.2 <- lm(VSI ~ cumulative_total_cases_per_hundred_last , data = data.train)
summary(lm.covid.1.refined.2)
```

Cumulative deaths per hundred prior to vaccination start, total number of cases and new deaths last reported prior to vaccination start appear to significantly impact `VSI`. After refining the model, the number of cases last reported prior to vaccination start appears to be the most important factor influencing `VSI`. Therefore, vaccination success seems influenced by a country's number of cases shortly before starting its vaccination campaign, where a higher number of cases leads to a better vaccination score. This could be explained by the fact that people may have a stronger vaccination incentive when COVID cases are more frequent around them.  
...
* `Electricity.Access_2018` - latest reported electricity access, may influence theinfrastructure available for vaccination
* `GDP.PerCapita.Current_2018` - latest GDP per capita
* `UrbanPopulation.Prop_3yr` - how urban is the population, averaged between 2015-2018
* `LifeExpenctancy_2018` - latest life expectancy
* `CompulsoryEducationDurationYears_3yr` - level of education, averaged between 2015-2018
* `WomenInBusinessLawIndex_2018` - latest proportion of women working in law and business fields
...
# remove non-significant variables
lm.political.1.refined <- lm(VSI ~ WorldRegion + LifeExpenctancy_2018 + IndividualsUsingInternet_2018, data = data.train)
Anova(lm.political.1.refined)
lm.political.1.refined.2 <- lm(VSI ~ LifeExpenctancy_2018 + IndividualsUsingInternet_2018, data = data.train)
summary(lm.political.1.refined.2)
lm.political.1.refined.3 <- lm(VSI ~ LifeExpenctancy_2018, data = data.train)
summary(lm.political.1.refined.3)
```

Life expectancy, world region and the number of individuals using internet (in 2018) seem to influence VSI. After refining the model by removing non-significant variables, life expectancy alone seemed to best explain VSI. 
...
# remove non-significant variables
lm.full.refined <- lm(VSI ~ LifeExpenctancy_2018,
             data = data.train)
Anova(lm.full.refined)

# plotting model diagnosis plots
plot(lm.full.refined, 1:2)
...
lm.full.refined.summ <- summary(lm.full.refined)
pval <- lm.full.refined.summ$coefficients[["LifeExpenctancy_2018", "Pr(>|t|)"]]

# get life expectancy correlation with VSI
cor_value <- cor(data.train$LifeExpenctancy_2018, data.train$VSI, use = "complete.obs")

plot_VSI_lifexp <- df %>% # using full df here to include country information
  ggplot(aes(x = LifeExpenctancy_2018, y = VSI)) +
  geom_point(aes(group = country, color = country), show.legend = FALSE) +
  geom_smooth(method = "lm", color = "grey") +
  theme_bw() +
  ggtitle("Association between life expectancy and VSI") +
  xlab("Life expectancy (2018)") +
  annotate("text", x = 56, y = max(df["VSI"]) - 0.5, # position
           size = 4, # font size
...
### Selecting the "best" WSD dataset 

In this second approach, we determine which of the created world sustainability measures (`WSD`) and `COVID` measures are most informative in relation to `VSI`. We try the same models on wsd data from 2018, wsd over the last 3 years, wsd average and wsd change between 2001 and 2018 to see which data best explains VSI. 


#### WSD - 2018

Let's first look at summary `WSD` measures from 2018 (i.e., the latest reported measures). 
...
data.train_2018 %>% 
  ...
  select(-contains(c("deaths", "cases"))) %>% # focusing on WSD measures only
...
correlations <- data.train_2018 %>% 
  select(-VSI) %>%
  select(-contains(c("deaths", "cases"))) %>%
  select_if(., is.numeric) %>%
  cor(., use = "complete.obs")
...
data.train_2018.sub <- data.train_2018 %>%
  select(-c(date_last, WorldRegion, # not involved in variable selection for the 2018 dataset
...
# internet use
lm.2018.1 <- lm(VSI ~ IndividualsUsingInternet_2018, data = data.train_2018.sub)
summary(lm.2018.1)
# internet significantly impacts VSI

# regime type
lm.2018.2 <- lm(VSI ~ RegimeType_2018, data = data.train_2018.sub)
summary(lm.2018.2)
# regime type does not significantly impact VSI

# urban population percentage
lm.2018.3 <- lm(VSI ~ UrbanPopulation.Prop_2018, data = data.train_2018.sub)
summary(lm.2018.3)
# urban population significantly impacts VSI

# years of compulsory education
lm.2018.4 <- lm(VSI ~ CompulsoryEducationDurationYears_2018, data = data.train_2018.sub)
summary(lm.2018.4)
# compulsory education does not significantly impact VSI

# GDP per capita
lm.2018.5 <- lm(VSI ~ GDP.PerCapita.Current_2018, data = data.train_2018.sub)
summary(lm.2018.5)
# GDP per capita significantly impacts VSI

# Life expectancy
lm.2018.6 <- lm(VSI ~ LifeExpenctancy_2018, data = data.train_2018.sub)
summary(lm.2018.6)
# Life expectancy  significantly impacts VSI
...
lm.2018.full <- lm(VSI ~ ., data = data.train_2018.sub)
...
lm.2018.red <- lm(VSI ~ IndividualsUsingInternet_2018, data = data.train_2018.sub)
summary(lm.2018.red)
anova(lm.2018.full, lm.2018.red)

```

Models using single predictors indicate that `LifeExpenctancy_2018`, `UrbanPopulation.Prop_2018`, `GDP.PerCapita.Current_2018` and `IndividualsUsingInternet_2018` each have significant effects on `VSI`. However, in a full model including all 2018 variables, `IndividualsUsingInternet_2018` is the only significant variable. In addition, there is no evidence to suggest that the full model better explains the data than the reduced model (p=0.07). Thus, internet usage alone is sufficient to explain `VSI` in this dataset. 

#### WSD - 3 year average


Let's now look at summary `WSD` measures averaged over a 3 year period from 2015-2018. 
...
# some variables are perfectly or almost perfectly correlated. We will trim variables that have correlations over 0.8 to avoid multi-collinearity. 
data.train_3yr.sub <- data.train_3yr %>%
  select(-c(date_last, WorldRegion, # not involved in variable selection for the 3yr dataset
            Electricity.Access_3yr, # LifeExpenctancy corr
            ImportGoodsServices.GDP_3yr, # ExportGoodsServices.GDP corr
            Trade.GDP_3yr, # ExportGoodsServices.GDP
            TotalNaturalResources.GDP_3yr, # SavingNaturalResourceDepletion
            GrossNationalExpenditure.GDP_3yr, # FinalConsumptionExpenditure.GDP
            RuralPopulation.Prop_3yr # UrbanPopulation.Prop corr
            )) %>%
  select(-contains(c("deaths", "cases"))) # focusing on WSD measures only

# # trying a few models with variables that should make sense...
# internet use
lm.3yr.1 <- lm(VSI ~ IndividualsUsingInternet_3yr, data = data.train_3yr.sub)
summary(lm.3yr.1)
# internet significantly impacts VSI

# regime type
lm.3yr.2 <- lm(VSI ~ RegimeType_3yr, data = data.train_3yr.sub)
summary(lm.3yr.2)
# regime type does not significantly impact VSI

# urban population percentage
lm.3yr.3 <- lm(VSI ~ UrbanPopulation.Prop_3yr, data = data.train_3yr.sub)
summary(lm.3yr.3)
# urban population significantly impacts VSI

# years of compulsory education
lm.3yr.4 <- lm(VSI ~ CompulsoryEducationDurationYears_3yr, data = data.train_3yr.sub)
summary(lm.3yr.4)
# compulsory education does not significantly impact VSI

# GDP per capita
lm.3yr.5 <- lm(VSI ~ GDP.PerCapita.Current_3yr, data = data.train_3yr.sub)
summary(lm.3yr.5)
# GDP per capita significantly impacts VSI

# Life expectancy
lm.3yr.6 <- lm(VSI ~ LifeExpenctancy_3yr, data = data.train_3yr.sub)
summary(lm.3yr.6)
# Life expectancy  significantly impacts VSI
...
lm.3yr.7 <- lm(VSI ~ LifeExpenctancy_3yr +
               ...
               data = data.train_3yr.sub)
summary(lm.3yr.7)
# only Life Expectancy is useful here, It seems like this variable may be sufficient at predicting VSI. 

# fit model (without any country/continent info)
lm.3yr.full <- lm(VSI ~ ., data = data.train_3yr.sub)
summary(lm.3yr.full)

# only LifeExpenctancy_3yr shows a significant effect on VSI. Trying a reduced model with only this variable
lm.3yr.red <- lm(VSI ~ IndividualsUsingInternet_3yr + Adj.SavingNetCO2Damage_3yr, data = data.train_3yr.sub)
summary(lm.3yr.red)
anova(lm.3yr.full, lm.3yr.red)

```

Models using single predictors indicate that `LifeExpenctancy_3yr`, `UrbanPopulation.Prop_3yr`, `GDP.PerCapita.Current_3yr` and `IndividualsUsingInternet_3yr` each have significant effects on `VSI`. However, in a full model including all 3 year average variables, `IndividualsUsingInternet_3yr` and `Adj.SavingNetCO2Damage_3yr` are the only significant variables. In addition, there is no evidence to suggest that the full model better explains the data than the reduced model (p=0.07). Thus, internet usage and adjusted savings (a measure that monitors whether savings and investment in a country compensate for depreciation of natural and physical capital) alone are sufficient to explain `VSI` in this dataset. 
...
data.train_avg.sub <- data.train_avg %>%
  select(-c(date_last, nYears_avg, WorldRegion,  # not involved in variable selection for the avg dataset
            Electricity.Access_avg, # LifeExpenctancy
            TotalNaturalResources.GDP_avg, # SavingNaturalResourceDepletion
            ImportGoodsServices.GDP_avg, # ExportGoodsServices.GDP
            Trade.GDP_avg, # ExportGoodsServices.GDP
            GrossNationalExpenditure.GDP_avg, # FinalConsumptionExpenditure.GDP
            RuralPopulation.Prop_avg, # UrbanPopulation.Prop
            GDP.PerCapita.Current_avg # IndividualsUsingInternet
            )) %>%
  select(-contains(c("deaths", "cases"))) # focusing on WSD measures only

# # trying a few models with variables that should make sense...
# internet use
lm.avg.1 <- lm(VSI ~ IndividualsUsingInternet_avg, data = data.train_avg.sub)
summary(lm.avg.1)
# internet significantly impacts VSI

# regime type
lm.avg.2 <- lm(VSI ~ RegimeType_avg, data = data.train_avg.sub)
summary(lm.avg.2)
# regime type does not significantly impact VSI

# urban population percentage
lm.avg.3 <- lm(VSI ~ UrbanPopulation.Prop_avg, data = data.train_avg.sub)
summary(lm.avg.3)
# urban population significantly impacts VSI

# years of compulsory education
lm.avg.4 <- lm(VSI ~ CompulsoryEducationDurationYears_avg, data = data.train_avg.sub)
summary(lm.avg.4)
# compulsory education significantly impacts VSI

# GDP per capita
lm.avg.5 <- lm(VSI ~ GDP.Current_avg, data = data.train_avg.sub)
summary(lm.avg.5)
# GDP significantly impacts VSI

# Life expectancy
lm.avg.6 <- lm(VSI ~ LifeExpenctancy_avg, data = data.train_avg.sub)
summary(lm.avg.6)
# Life expectancy  significantly impacts VSI
...
lm.avg.7 <- lm(VSI ~ LifeExpenctancy_avg +
               ...
               data = data.train_avg.sub)
summary(lm.avg.7)
# only Life Expectancy is useful here, It seems like this variable may be sufficient at predicting VSI. 

# fit model (without any country/continent info)
lm.avg.full <- lm(VSI ~ ., data = data.train_avg.sub)
summary(lm.avg.full)

# only LifeExpenctancy_avg shows a significant effect on VSI. Trying a reduced model with only this variable
lm.avg.red <- lm(VSI ~ Adj.SavingNetForestDepletion_avg + Adj.SavingParticulateEmissions_avg, data = data.train_avg.sub)
summary(lm.avg.red)
anova(lm.avg.full, lm.avg.red)

# also comparing full to lm.avg.7
anova(lm.avg.full, lm.avg.7)

```

Models using single predictors indicate that `LifeExpenctancy_avg`, `UrbanPopulation.Prop_avg`, `CompulsoryEducationDurationYears_avg`, `GDP.Current_avg` and `IndividualsUsingInternet_avg` each have significant effects on `VSI`. In a full model including all 3 year average variables, `Adj.SavingNetForestDepletion_avg` and `Adj.SavingParticulateEmissions_avg` are the only significant variables. That said, the full model better explains the data than the reduced model (p<0.001) including only those 2 variables. In addition, the model including `LifeExpenctancy_avg`, `UrbanPopulation.Prop_avg`, `CompulsoryEducationDurationYears_avg`, `GDP.Current_avg` and `IndividualsUsingInternet_avg` appears to best explain `VSI` (p = 0.07). Thus, the above variables appear important in explaining `VSI` in this dataset. 


#### WSD - 17 year change


Let's now look at summary `WSD` measures indicating change between 2001 and 2018.
...
# some variables are perfectly or almost perfectly correlated. We will trim variables that have correlations over 0.8 to avoid multi-collinearity. 
data.train_diff.sub <- data.train_diff %>%
  select(-c(date_last, WorldRegion,  # not involved in variable selection for the diff dataset
            Trade.GDP_rel.diff, # ExportGoodsServices.GDP_rel.diff
            GrossNationalExpenditure.GDP_rel.diff, # FinalConsumptionExpenditure.GDP_rel.diff
            GDP.PerCapita.Current_rel.diff # GDP.Current_rel.diff
            )) %>%
  select(-contains(c("deaths", "cases"))) # focusing on WSD measures only

# # trying a few models with variables that should make sense...
# internet use
lm.diff.1 <- lm(VSI ~ IndividualsUsingInternet_rel.diff, data = data.train_diff.sub)
summary(lm.diff.1)
# internetusage has a significant effect on VSI

# regime type
lm.diff.2 <- lm(VSI ~ RegimeType_rel.diff, data = data.train_diff.sub)
summary(lm.diff.2)
# regime type has no effect on VSI

# urban population percentage
lm.diff.3 <- lm(VSI ~ UrbanPopulation.Prop_rel.diff, data = data.train_diff.sub)
summary(lm.diff.3)
# urban population has no effect on VSI

# years of compulsory education
lm.diff.4 <- lm(VSI ~ CompulsoryEducationDurationYears_rel.diff, data = data.train_diff.sub)
summary(lm.diff.4)
# compulsory education has no effect on VSI

# GDP per capita
lm.diff.5 <- lm(VSI ~ GDP.Current_rel.diff, data = data.train_diff.sub)
summary(lm.diff.5)
# GDP has no effect on VSI

# Life expectancy
lm.diff.6 <- lm(VSI ~ LifeExpenctancy_rel.diff, data = data.train_diff.sub)
summary(lm.diff.6)
# Life expectancy has a significant effect on VSI

# fit model (without any country/continent info)
lm.diff.full <- lm(VSI ~ ., data = data.train_diff.sub) 
summary(lm.diff.full) # this still outputs NAs due to a large amount of NAs in the dataset. 

```

When considering single predictors, internet usage has a significant effect on VSI. When considering all variables, the full model will output NAs as measures computed as the 17 year difference relative to baseline appear to have many NAs. Therefore, 17 year difference measures do not seem suitable to use for further modeling. 


### Selecting the "best" COVID dataset

Finally, let's look at summary `COVID` cases and deaths measures to determine which ones are most informative. 

#### COVID prevac

We first consider covid `prevac` summary measures, which contain the latest reported cases and deaths measures before each country's vaccination start. 
...
# some variables are perfectly or almost perfectly correlated. We will trim variables that have correlations over 0.8 to avoid multi-collinearity. 
data.train_prevac.sub <- data.train %>%
  # select(contains(c("deaths", "cases"))) # focusing on WSD measures only
  select(VSI, contains(c("prevac"))) %>%
  select(-c(cumulative_total_cases_prevac, # cumulative_total_deaths_prevac
            active_cases_prevac, # daily_new_deaths_prevac
            daily_new_cases_per_hundred_prevac, # weekly_new_deaths_per_hundred_prevac
            cumulative_total_cases_per_hundred_prevac, # cumulative_total_deaths_per_hundred_prevac
            weekly_new_cases_prevac, # monthly_new_casesprevac
            weekly_new_cases_prevac, # monthly_new_casesprevac
            monthly_new_cases_prevac, # monthly_new_deaths_prevac
            weekly_new_deaths_per_hundred_prevac, # monthly_new_deaths_per_hundred_prevac
            weekly_new_cases_per_hundred_prevac, # active_cases_per_hundred_prevac
            daily_new_cases_prevac, # cumulative_total_cases_prevac
            ))


# fit full model 
lm.prevac.full <- lm(VSI ~ ., data = data.train_prevac.sub) 
summary(lm.prevac.full) 

```

None of the prevac variables have a significant effect. 

#### COVID max

We next consider covid `max` summary measures, which contain the peak of reported cases and deaths measures before each country's vaccination start. 
...
data.train_prevac.sub <- data.train %>%
  # select(contains(c("deaths", "cases"))) # focusing on WSD measures only
  select(VSI, contains(c("max"))) %>%
  select(-c(daily_new_cases_max,
            daily_new_deaths_max,
            daily.new.cases.per.100_max,
            daily.new.deaths.per.100_max,
            weekly_new_cases_max,
            weekly_new_deaths_max,
            weekly.new.cases.per.100_max, 
            weekly.new.deaths.per.100_max,
            monthly_new_cases_max,
            monthly_new_deaths_max
            ))
...
None of the max variables have a significant effect. 


#### COVID last

Finally, we consider covid `last` summary measures, which contain the peak of reported cases and deaths measures before each country's vaccination start. 
...
# some variables are perfectly or almost perfectly correlated. We will trim variables that have correlations over 0.8 to avoid multi-collinearity. 
data.train_last.sub <- data.train %>%
  # select(contains(c("deaths", "cases"))) # focusing on WSD measures only
  select(VSI, contains(c("last"))) %>%
  select(-c(daily_new_cases_last,
            active_cases_last,
            daily_new_cases_per_hundred_last,
            weekly_new_cases_last,
            weekly_new_cases_per_hundred_last,
            monthly_new_cases_last,
            daily_new_deaths_last,
            daily_new_deaths_per_hundred_last,
            monthly_new_deaths_last,
            weekly_new_deaths_last,
            weekly_new_deaths_per_hundred_last
            ))
...
None of the last reported variables have a significant effect. 
