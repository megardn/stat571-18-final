...
pacman::p_load(tidyverse, dplyr, ggplot2, ggthemes, data.table, lubridate,
               ...
               plotly, ggpubr, vistime, skimr, glmnet, dgCMatrix, leaps, car, keras, neuralnet, tensorflow, missForest, corrplot, tree, randomForest, fastDummies)
```
...
While an advantage of using a single tree is that we can see which variables are diving our prediction, Random Forest may provide better results at the cost of interpretability. Accordignly, now we will tune and train a Random Forest model. 
...
```{r}
fit1 <- regsubsets(VSI ~., data.train , nvmax=25, method="forward", really.big = TRUE)
f.e <- summary(fit1)
...
fit1.lm <- lm(VSI ~ LifeExpenctancy_avg, data.train)
Anova(fit1.lm)
```
...
data.nn.train <- data.train.test %>% 
  ...
  select_if(funs(!is.Date(.))) #drop dates, can't be imputed
...
dim(nn.trainimp) #imputing gives us 89 cols to work with
dim(data.nn.train[ , colSums(is.na(data.nn.train)) == 0]) #as opposed to dropping all NA cols, which only gives us 57
dim(na.omit(data.nn.train)) #or dropping rows with NAs, which would mean only 19 countries

#set dummy variables for categorical
nn.train.final <- dummy_cols(nn.trainimp, remove_first_dummy = TRUE) %>% #setting remove_first_dummy = TRUE to have base levels
 select_if(funs(!is.factor(.))) #drop original factors

## training input/y: need to be matrix/vector
data_xtrain <- nn.train.final %>% dplyr::select(-VSI) %>% as.matrix()  #dim(data3_xtrain)
data_ytrain <- nn.train.final %>% dplyr::select(VSI) %>% as.matrix()  # find y

# ## validation input/y - figure this out with impute/dummy vars later!
...
model <- keras_model_sequential() %>%
  ...
  layer_dropout(0.25) %>% # drop 20% - https://stackoverflow.com/questions/37232782/nan-loss-when-training-regression-network
  layer_dense(units = 26, activation = "relu") %>%  
  layer_dropout(0.25) %>% # drop 20%
...
#compile model B (based on https://tensorflow.rstudio.com/tutorials/beginners/basic-ml/tutorial_basic_regression/)
model %>%
  compile(
    loss = "mse",
...
# #compile model C (based on https://towardsdatascience.com/keras-and-r-predicting-blood-glucose-levels-with-the-sequential-model-596efe89a6b8)
# model %>% compile(
#   loss = 'mean_squared_error',
#   optimizer = 'adam',
#   metrics = c('mae')
...
good (?) params: 
12x12x1, model B, epochs=150 & batch size=50 @ epoch 20
  - loss = 1.95, val_loss = 3.73
12x12x1, model B, epochs=150 & batch size=32 @ epoch 20
  - loss = 1.92, val_loss = 3.71
12x12x1, model B, epochs=150 & batch size=24 @ epoch 20
  - loss = 1.73, val_loss = 3.46
12x12x1, model B, epochs=150 & batch size=18 @ epoch 20
  - loss = 1.48, val_loss = 3.32
12x12x1, model B, epochs=150 & batch size=12 @ epoch 20
  - loss = 1.67, val_loss = 2.93
*16x8x1, model B, epochs=150 & batch size=12 @ epoch 20*
  - loss = 1.23, val_loss = 2.1
*16x8x1, model B, epochs=150 & batch size=24 @ epoch 20*
  - loss = 1.1, val_loss = 2.4
16x8x1, model B, epochs=150 & batch size=24 @ epoch 20
  - loss = 1.15, val_loss = 2.65
16x8x1, model C, epochs=150 & batch size=36 @ epoch 20
  - loss = 1.3, val_loss = 2.83
16x8x1, model C, epochs=150 & batch size=24 @ epoch 20
  - loss = 1.18, val_loss = 2.41
16x8x1, model C, epochs=150 & batch size=12 @ epoch 20
  - loss = 1.24, val_loss = 2.06 - looks gross though
16x8x1, model B, epochs=150 & batch size=12 @ epoch 20 w / dropout
  - loss = 1.75, val_loss = 2.2
24x16x1, model B, epochs=150 & batch size=12 @ epoch 20
  - loss = 1.16, val_loss = 1.94
24x16x1, model B, epochs=150 & batch size=12 @ epoch 20 w/ dropout
  - loss = 1.81, val_loss = 1.93
24x16x1, model B, epochs=150 & batch size=24 @ epoch 20
  - loss = 0.83, val_loss = 3.24
24x16x1, model B, epochs=150 & batch size=24 @ epoch 20 w/ dropout 25%
  - loss = 1.78, val_loss = 2.4
