...
##dat vac start doesn't actually calculate - just earliest date that vaccines were reported but many countries dont have data until a while after starting, ie 70% pop is already vaccinated at day 1
vac_start_df <- covid %>%
...
#adding date of first vaccination for each country to the covid df so it can be used as a baseline later - there may be a more elegant way of doing this but im tired
...
covid_prevac <- covid %>%
  ...
  slice_tail(n=1) %>%
  ungroup()
covid_prevac <- covid_prevac[, which(colMeans(!is.na(covid_prevac)) > 0.5)] #remove cols with NA > 50%
