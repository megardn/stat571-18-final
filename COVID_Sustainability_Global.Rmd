...
pacman::p_load(tidyverse, dplyr, ggplot2, ggthemes, data.table, lubridate,
               ...
               plotly, ggpubr, vistime, skimr, glmnet, dgCMatrix, leaps, car, keras, neuralnet, tensorflow, missForest, corrplot)
```
...
lm.econ.1.refined <- lm(data = data.train, formula = VSI ~ GDP.PerCapita.Current_avg)
...
* `monthly_new_deaths_per_hundred_prevac` - last reported monthly death rate reached prior to vaccination start
...
lm.covid.1 <- lm(VSI ~ cumulative_total_cases_per_hundred_prevac +
                   ...
                   monthly_new_cases_per_hundred_prevac +
                   monthly_new_deaths_per_hundred_prevac, data = data.train)
summary(lm.covid.1)
...
lm.covid.f2 <- update(lm.covid.f, .~. + cumulative_total_cases_per_hundred_prevac)
...
**Question - do we want to be doing this on the full df because it has country but also includes the test and val subsets? Follow-up, do we want to run the final model separately on the test subset? Was it a bad idea and/or unnecessary to subset the data in the first place?**
...
plot_VSI_lifexp <- final.df %>% # using full df here to include country information 
  ...
  theme_bw() +
  ggtitle("Association between 2018 life expectancy and VSI") +
...
correlations <- data.train_2018 %>% 
  select(-VSI) %>%
  select(-contains(c("deaths", "cases"))) %>% #remove covid vars
  select_if(., is.numeric) %>%
  cor(., use = "complete.obs")
...
data.train_2018.sub <- data.train_2018 %>%
  select(-c(WorldRegion, # not involved in variable selection for the 2018 dataset
...
### MG removing correlated variables
using method from https://statisticsglobe.com/remove-highly-correlated-variables-from-data-frame-r to automatically remove highly correlated variables w/in WSD and COVID predictors
...
#remove vars at abs(0.95) threshold - keeping high to preserve as many variables as possible (also b/c we expect things to be correlated IRL)
data.train.wsd.nocor <- data.train[ , !apply(wsd.corr,    # Remove highly correlated variables
                           2,
                           function(x) any(abs(x) > 0.95))]
head(data.train.wsd.nocor)
```
Now w/in COVID

```{r}
#correlations
covid.corr <- data.train %>% 
  select(-VSI) %>% # rm y
  select(contains(c("deaths", "cases"))) %>% #only covid vars
  select_if(., is.numeric) %>%
  cor(., use="pairwise.complete.obs")
#removing upper triangle
...
#remove vars at abs(0.95) threshold - keeping high to preserve as many variables as possible (also b/c we expect things to be correlated IRL)
data.train.covid.nocor <- data.train[ , !apply(covid.corr,    # Remove highly correlated variables
                           2,
                           function(x) any(abs(x) > 0.95) | )]
head(data.train.covid.nocor)
```

Apply both for full predictor df

```{r}
covid.list <- apply(covid.corr, 2, function(x) any(abs(x) > 0.95))
wsd.list <-  apply(wsd.corr, 2, function(x) any(abs(x) > 0.95))
cor.list <- c(covid.list, wsd.list)
data.train.nocor <- data.train[ , !cor.list]
head(data.train.nocor)
```
