...
* In the second approach, we are running more inclusive models on distinct `WSD` summary measures (2018 data, 3 year average data, etc.) to determine which summary measures may be most informative in predicting `VSI`. 
...
In this second approach, we determine which of the created world sustainability measures (`WSD`) are most informative in relation to `VSI`. We try the same models on wsd data from 2018, wsd over the last 3 years, wsd average and wsd change between 2001 and 2018 to see which data best explains VSI. 
...
data.train_2018.sub <- data.train_2018 %>%
  select(-c(date_last, WorldRegion,
...
# Life expectancy
lm.2018.6 <- lm(VSI ~ Electricity.Access_2018, data = data.train_2018)
summary(lm.2018.6)
...
lm.2018.red <- lm(VSI ~ LifeExpenctancy_2018, data = data.train_2018.sub)
summary(lm.2018.red)
anova(lm.2018.full, lm.2018.red)

```

--- From here and below needs updating
