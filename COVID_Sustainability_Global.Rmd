...
* `WorldRegion`
* `Electricity.Access_2018` - since this would be related to infrastructure used for vaccination
...
lm.poltical.1 <- lm(VSI ~ WorldRegion +
...
summary(lm.poltical.1)

# remove non-significant variables
lm.poltical.1.refined <- lm(VSI ~ WorldRegion + LifeExpenctancy_2018 + IndividualsUsingInternet_2018, data = data.train)
Anova(lm.poltical.1.refined)
lm.poltical.1.refined.2 <- lm(VSI ~ LifeExpenctancy_2018 + IndividualsUsingInternet_2018, data = data.train)
summary(lm.poltical.1.refined.2)
lm.poltical.1.refined.3 <- lm(VSI ~ LifeExpenctancy_2018, data = data.train)
summary(lm.poltical.1.refined.3)
```
...
### Narrowing down the "best" dataset 


running a linear model that includes all variables of the dataframe 

Next, we will try to determine which of the created world sustainability measures 
are trying a few simple linear models by selecting variables based on domain knowledge. We try the same models on wsd data from 2018, wsd over the last 3 years, wsd average and wsd change between 2001 and 2018 to see which data best explains VSI. 
