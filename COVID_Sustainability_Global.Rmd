...
wsd.df_list <- list(wsd.change, wsd.3yr, wsd.2018, wsd.avg) #dfs to merge, ordered purposefully for removing correlated predictors later on!
...
* In the second approach, we remove highly colinear WSD and COVID predictors (abs value > 0.95) before running more inclusive, data-driven models using LASSO. 
...
Before running our data-driven models, we'll automatically remove one of each pair of highly-colinear predictors within WSD and uor COVID pre-vaccine cases & deaths datasets. While the removal is unsupervised based on our specified 0.95 threshold, our dataframe is structured such that variables we believe will be more predictive or complete from a prior knowledge (2018 values or values averaged over all years) will be prioritized for selection over variables that are less likely to be relevant (such as change over the 17 year WSD period).

```{r}
#method from https://statisticsglobe.com/remove-highly-correlated-variables-from-data-frame-r
#correlations
wsd.corr <- data.train %>% 
  select(-VSI) %>% # rm y
...
cor.list <- c(covid.list, wsd.list) #list highly correlated variables
data.train.nocor <- data.train[ , !cor.list]
head(data.train.nocor)

#see how many of each type of summary var were preserved
data.train.nocor %>% dplyr::select(contains("_max")) %>% ncol()
data.train.nocor %>% dplyr::select(contains("_prevac")) %>% ncol()
data.train.nocor %>% dplyr::select(contains("_rel.diff")) %>% ncol()
data.train.nocor %>% dplyr::select(contains("_diff")) %>% ncol()
data.train.nocor %>% dplyr::select(contains("_3yr")) %>% ncol()
data.train.nocor %>% dplyr::select(contains("_2018")) %>% ncol()
data.train.nocor %>% dplyr::select(contains("_avg")) %>% ncol()
```
...
THIS CHUNK REMOVES BAD VARIABLES - FEEL FREE TO MOVE IT ELSEWEHERE 
```{r}
...
Our mean squared prediction error using a single tree is `tree.mse`! That is quite high, though not surprising, given the complexity of the question. Now, let's try something more robust. While an advantage of using a single tree is that we can see which variables are diving our prediction, Random Forest may provide better results at the cost of interpretability.  
...
fit.1.rf <- randomForest(VSI~., cleaned.data.train, mtry=6, ntree=500)
...
```{r}
set.seed(10)
max.mtry <- ncol(cleaned.data.train)  
error.p <- 1:max.mtry 
for (p in 1:max.mtry)  
{
  rf.temp <- randomForest(VSI~., cleaned.data.train, mtry=p, ntree=100)
...
```{r}
set.seed(2) 
fit.final.rf <- randomForest(VSI~., cleaned.data.train, mtry=20, ntree=100)
plot(fit.final.rf)
```
